    ...
    glProgram_CopyFrameToBackBuffer(std::move(ShaderCompiler::singleton().compileProgramFromFiles("copy-forward-framebuffer", QDir(GLRT_SHADER_DIR"/materials/implementation"))))
{
  ...
  MaterialState::Flags skyFlags = MaterialState::Flags::SCREENSPACE_POS_ATTRIBUTES | MaterialState::Flags::COLOR_WRITE | MaterialState::Flags::DEPTH_TEST;
  ...
  appendMaterialState(&framebuffer, {SKY}, Pass::FORWARD_PASS, skyShader, skyFlags);
